...
class UsageDimensionCalculator(IDimensionCalculator):
    ...
    def calculate(self, features: dict[str, Any]) -> FinancialDimension:
        ...
        vol_score = min(50.0, math.log1p(max(0.0, tx_vol)) * 7.5)
        score_raw = min(100.0, max(0.0, freq_score + vol_score))
...
class ResilienceDimensionCalculator(IDimensionCalculator):
    ...
    def calculate(self, features: dict[str, Any]) -> FinancialDimension:
        ...
        score_raw = (40.0 if fallback_avail else 0.0) + min(60.0, math.log1p(max(0.0, reserve_usd)) * 9.0)
        score_raw = min(100.0, max(0.0, score_raw))
